...
class PyIconButton(QPushButton):
    ...
    def paintEvent(self, event):
        ...
        # DRAW ICONS
        self.icon_paint(paint, self._set_icon_path, rect, self._icon_margin)
    ...
    # ///////////////////////////////////////////////////////////////
    def icon_paint(self, qp, image, rect, margin):
        # Load the SVG image using QSvgRenderer
        svg_renderer = QSvgRenderer(image)
        
        # Create a new QPixmap to hold the rendered SVG image
        pixmap = QPixmap(rect.size())
        pixmap.fill(Qt.transparent)  # Fill with transparent color
        
        # Use QPainter to render the SVG onto the QPixmap
        painter = QPainter(pixmap)
        svg_renderer.render(painter)
        
        # Scale the pixmap while keeping the aspect ratio
        max_dimension = max(rect.width(), rect.height()) - margin
        scaled_pixmap = pixmap.scaled(QSize(max_dimension, max_dimension), Qt.KeepAspectRatioByExpanding, Qt.SmoothTransformation)
        

        
        # Draw the pixmap onto the target rectangle
        qp.drawPixmap(rect.x() + (rect.width() - scaled_pixmap.width()) // 2,
                    rect.y() + (rect.height() - scaled_pixmap.height()) // 2,
                    scaled_pixmap)
